 ...
 * @return {number}
 */
function getDiffAge(human1, human2) {
    return human1.born - human2.born;
}
 ...
 * @param {TSex} [sex]
 *
 * @return {THuman | undefined}
 */
function getHuman(humanName, sex) {
    return ANCESTRY_DATA.find(function(human) {
        return human.name === humanName && (!sex || human.sex === sex);
    });
}
 ...
 * @returns {number}
 */
function getAgeDiffWithMother(human) {
    if (!human.mother) {
        return null;
    }

    const mother = getHuman(human.mother, 'f');

    if (!mother) {
        return null;
    }

    return getDiffAge(human, mother);
}
 ...
 * @returns {number}
 */
function getAgeDiffBetweenMothersAndChildren() {
    return getAverage(
        ANCESTRY_DATA
            .map(function(human) {
                return getAgeDiffWithMother(human);
            })
...
console.log( getAgeDiffBetweenMothersAndChildren() );
